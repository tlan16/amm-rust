FROM --platform=linux/amd64 rust:1.83.0-bookworm AS builder-x86_64-unknown-linux-gnu
...
FROM --platform=linux/arm64 rust:1.83.0-bookworm AS builder-aarch64-unknown-linux-gnu
# OS dependencies
ENV DEBIAN_FRONTEND=noninteractive
RUN apt-get update && apt-get install -y \
    libxdo-dev \
...
FROM rust:1.83.0-bookworm AS apple-base
